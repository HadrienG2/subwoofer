...
- "L1cache" behaves in a standard way by loading a respectably-sized set of data
  ...
  L1 cache's bandwidth or to be limited by its latency (both of which are
  thankfully unlikely on common high-performance CPU microarchitectures).
- To work around this, "Nregisters" uses a much tinier set of data inputs that
  can stay resident in CPU registers for the entire duration of the benchmark.
  This is the fastest possible source of data on the CPU side, unlike the L1
  cache it cannot ever become a bottleneck. But there is a big price to pay for
...
- If "Nregisters" is a bit faster than "L1cache" (say, they are within 10% of
  each other), its output might be more faithful to the impact of subnormal
  arithmetic on maximally optimized code because it is not polluted by memory
  loads, it contains only ~free reg-reg moves and the desired arithmetic.
- If "Nregisters" is a lot faster than "L1cache" (say, 2x faster), or even
  worse, slower, you should disregard its output as suspicious by default unless
  you are ready to take the time to carefully cross-check that the output
  machine code generated by rustc is right, that the CPU micro-architecture
